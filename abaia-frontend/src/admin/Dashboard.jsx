...
const Dashboard = () => {
  ...
  const totalVat = orders.reduce(
    (s, o) => s + parseFloat(o.vat_amount || 0),
    0,
  );
    ...
    try {
      await api.put(
        `/api/auth/users/${editingUser.id}`,
        {
          ...
          profile_pic: editingUser.profile_pic,
        },
        authHeaders,
      );
      setEditingUser(null);
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
          <div className="stat bg-white dark:bg-base-100 shadow-sm rounded-xl border border-base-200 p-4 sm:p-6">
            ...
            <div className="stat-value text-xl sm:text-2xl text-success font-bold">
              {Number(totalSales).toLocaleString()} {t("sar")}
            </div>
          </div>
          <div className="stat bg-white dark:bg-base-100 shadow-sm rounded-xl border border-base-200 p-4 sm:p-6">
            <div className="flex items-center gap-3 mb-2">
            ...
            <div className="stat-value text-xl sm:text-2xl text-warning font-bold">
              {Number(totalVat).toLocaleString()} {t("sar")}
            </div>
          </div>
          <div className="stat bg-white dark:bg-base-100 shadow-sm rounded-xl border border-base-200 p-4 sm:p-6">
            <div className="flex items-center gap-3 mb-2">
            ...
            <div className="stat-value text-xl sm:text-2xl text-info font-bold">
              {orders.length}
            </div>
          </div>
        </div>
        ...
        {/* QR Code */}
        <div className="card bg-white dark:bg-base-100 shadow-sm rounded-xl border border-base-200 p-4 sm:p-6 mb-6">
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
            <div className="bg-white p-2 rounded-xl shadow-sm border border-base-200">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(typeof window !== "undefined" ? window.location.origin : "")}&bgcolor=ffffff&color=1a1a1a`}
                alt="QR Code"
            ...
            <div className="text-center sm:text-start">
              ...
              <p className="text-xs sm:text-sm text-base-content/50 mb-2">
                {lang === "ar"
                  ? "وجه كاميرا هاتفك نحو الرمز لفتح المتجر مباشرة"
                  : "Point your phone camera at the QR code to open the store"}
              </p>
              <p className="text-xs font-mono text-base-content/40 bg-base-200 px-3 py-1 rounded-full inline-block">
          ...
          <div className="bg-white dark:bg-base-100 rounded-xl shadow-sm border border-base-200 overflow-hidden">
            <div className="flex items-center justify-between p-4 sm:p-5 border-b border-base-200">
              <h3 className="text-base sm:text-lg font-bold">
                {t("allProducts")}{" "}
                <span className="text-base-content/40 text-sm font-normal">
                  ({products.length})
                </span>
              </h3>
              <button
              ...
              <div className="p-4 sm:p-5 border-b border-base-200 bg-base-100">
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewProduct({ ...newProduct, title_en: e.target.value })
                    }
                  />
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selected = styles.find(
                        (s) => s.ar === e.target.value,
                      );
                      setNewProduct({
                        ...newProduct,
                        style: e.target.value,
                        style_en: selected ? selected.en : "",
                      });
                    }}
                  >
                ...
                <div key={p.id} className="p-4 space-y-2">
                  <div className="flex items-center gap-3">
                    {p.image_url ? (
                      ...
                      <div className="w-14 h-14 rounded-lg bg-base-200 flex items-center justify-center text-xs text-base-content/30">
                        {t("noImage")}
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-base-content/50">
                        {p.price} {t("sar")} • {t("qtyPlaceholder")}:{" "}
                        {p.quantity}
                      </p>
                      <p className="text-xs text-base-content/40">
                        {t("size")}: {p.size} • {p.color}
                      </p>
                    </div>
                ...
                <tbody>
                  {products.length === 0 && (
                    <tr>
                      <td
                        colSpan={9}
                        className="text-center py-10 text-base-content/30"
                      >
                        {t("noProductsAdmin")}
                      </td>
                    </tr>
                    ...
                    >
                      ...
                      <td>
                        {p.image_url ? (
                          ...
                          <div className="w-12 h-12 rounded-lg bg-base-200 flex items-center justify-center text-xs text-base-content/30">
                            {t("noImage")}
                          </div>
                        )}
                      </td>
                      ...
                      <td>
                        <span
                          className={`badge badge-sm ${p.quantity > 0 ? "badge-success" : "badge-error"} gap-1`}
                        >
                          {p.quantity}
                        </span>
                      </td>
                      <td>
                        <div className="flex items-center gap-2">
                          <div
                            ...
                            style={{
                              backgroundColor: colorMap[p.color] || "#ccc",
                            }}
                          />
              ...
              {users.map((u) => (
                <div key={u.id} className="p-4 flex items-center gap-3">
                  {u.profile_pic ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-base-200 flex items-center justify-center text-sm text-base-content/30">
                      {u.name?.[0] || "?"}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    ...
                    <span
                      className={`badge badge-xs mt-1 ${u.role === "admin" ? "badge-primary" : "badge-ghost"}`}
                    >
                    ...
                    >
                      ...
                      <td>
                        {u.profile_pic ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-base-200 flex items-center justify-center text-xs text-base-content/30">
                            {u.name?.[0] || "?"}
                          </div>
                        )}
                      </td>
        ...
        <dialog className={`modal ${deleteTarget ? "modal-open" : ""}`}>
          <div className="modal-box rounded-xl shadow-2xl">
            ...
            <div className="modal-action gap-3">
              ...
              <button onClick={handleDelete} className="btn btn-error gap-1">
                <Trash2 size={16} /> {t("del")}
              </button>
            </div>
          </div>
          <form
            ...
            {editingProduct && (
              ...
              >
                ...
                <input
                  type="file"
                  accept="image/*"
                  className="file-input file-input-bordered w-full file-input-sm sm:col-span-2"
                  onChange={(e) => {
                    const file = e.target.files[0];
